package com.example.jingbiaowang.simplep2p.play;
...
import com.example.jingbiaowang.simplep2p.R;
...
public class PlayActivity extends FragmentActivity implements PeerConnectionClient.PeerConnectionEvents, PlayFragment.OnCallEvents, SocketResonseEvents.P2pSockeInternalEvents {
...
//                new UnhandledExceptionHandler(this));
    ...
    private void initialize() {
        ...
        P2pSocketClient.newInstance().setInternalEvents(this);
        ...
        intent.putExtra(EXTRA_CMDLINE, true);
        intent.putExtra(EXTRA_RUNTIME, 0);
        intent.putExtra(EXTRA_ROOMID, getIntent().getStringArrayExtra(EXTRA_ROOMID));
...
//            try {
//                P2pSocketClient.newInstance().close();
//            } catch (IOException e) {
//                e.printStackTrace();
//            }
//        }
    ...
    private void disconnectWithErrorMessage(final String errorMessage) {
        if (commandLineRun || !activityRunning) {
            ...
            LogCat.debug("disconnect 3");
            disconnect();
        } else {
            new AlertDialog.Builder(this, android.support.design.R.style.Base_Theme_AppCompat_Dialog_Alert)
    ...
    public void onLocalDescription(SessionDescription sdp) {
        ...
        if (getIntent().getBooleanExtra("call", true))
            P2pSocketClient.newInstance().call(getIntent().getStringExtra("to"), sdp.description, getIntent().getStringExtra("from"));
        else
            P2pSocketClient.newInstance().incomingCallResponse(getIntent().getStringExtra("from"), sdp.description, true);
    }
    ...
    protected void onDestroy() {
        P2pSocketClient.newInstance().setInternalEvents(null);
        super.onDestroy();
    }
    ...
    @Override
    public void onCallResponseOk(final String sdpAnwser) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                peerConnectionClient.setRemoteDescription(new SessionDescription(SessionDescription.Type.ANSWER, sdpAnwser));
            }
        });
    }

    @Override
    public void startCommunication(final String sdpAnwser, int msgCode) {
        ...
        if (msgCode == 1) {
            ...
            Toast.makeText(getApplicationContext(), "startCommunication error", Toast.LENGTH_SHORT).show();
        }
    }

    @Override
    public void onIceCandidateResponse(final com.sohu.kurento.bean.IceCandidate candidate) {

        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                peerConnectionClient.addRemoteIceCandidate(new IceCandidate(candidate.sdpMid, candidate.sdpMLineIndex, candidate.candidate));
            }
        });
    }
